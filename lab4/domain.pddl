(define (domain lunar) 
    ...
    (:predicates 
        (less ?n1 - position ?n2 - position) 
        (one-less ?n1 - position ?n2 - position) 
        (pos ?s - spacecraft ?x - position ?y - position) 
    ) 
     
    (:action move_down 
        :parameters (?s1 ?s2 - spacecraft  
                     ?sx ?s1y ?s2y ?new_s1y - position) 
